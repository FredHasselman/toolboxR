...
# ## An R Function Library foused for the ManyLabs1, ManyLabs2, RPP and Curate Science projects
# 
...
# 
# All the functions that are required are available in `C3PR.R`, a package will follow soon. 
...
in.PLOT <- function(useArial = T,afmPATH="~/Dropbox/Public"){
  ...
  ip <- .packages(all.available = T)
  need <- c("lattice","latticeExtra","gplots","ggplot2","grid","gridExtra","scales","beanplot","effects","RColorBrewer")
...
df.Clean <- function(df,Sep="."){
  ...
  colnames(df) <- nms
  df      <- select(df,which(nms%in%nms[nms!=""]))    
...
get.GoogleSheet <- function(url=NULL,study=c("ML1","ML2key","RPPdata")[3],Sep = "."){
  # url <- "https://docs.google.com/spreadsheet/ccc?key=10IXGYUvt9vb64FyXP2Wlf03X5lPo_AvhQOsNs6w84dk&single=true&gid=0&output=csv"
  switch(study,
         #    ML1 = url <- "https://docs.google.com/spreadsheets/d/19ay71M8jiqIZhSj3HR0vqaJUwAae1QHzBybjBu5yBg8&single=true&gid=0&output=csv",
         #   ML2key = url <- "https://docs.google.com/spreadsheets/d/19ay71M8jiqIZhSj3HR0vqaJUwAae1QHzBybjBu5yBg8&single=true&gid=0&output=csv",
...
# PLOTS -------------------------------------------------------------------
disp <- function(message='Hello world!',header=NULL){
...
theme.noax <- theme(line = element_blank(),
                    text  = element_blank(),
                    title = element_blank(),
                    plot.background = element_blank(),
                    panel.border = element_blank(),
...
set.Arial <- function(afmPATH){
  # Set up PDF device on MAC OSX to use Arial as a font in Graphs
    if (!"Arial" %in% names(pdfFonts())) {
     ...
     disp(header='useArial=TRUE',message='Please provide the path to the *.afm version of the Arial font family')
    }
}
...
get.results <- function(key,dfr,s,group=NULL){
  ...
  dfr <- tbl_df(dfr)
  cat('# Get info to create a dataset for the current study\n')
...
get.info <- function(keytable,cols){
  ...
  id.vars  <- which(cols%in%c(unlist(study.vars),'uID','.id','age','sex','race','born','born.par','born.par2','Language','source') )
...
varfun.Ross.2 <- function(vars=ML2.sr, DEMO=FALSE){
  # Analysis plan. An independent samples t-test will be conducted with participants’ choice
  ...
  if(DEMO==TRUE){
    ...
    warning('Ross.2: DEMO=TRUE',call. = FALSE)  
    return(list(Peers = rnorm(50),
                You   = c(rep(1,25),rep(2,25)),
                N     = c(50,50)))
...
varfun.Tversky.1 <- function(vars=ML2.sr){
  ...
  return(list(Long   = ML2.df[ML2.s],
              Short  = ML2.df[id.ValidS,ML2.in$study.vars$Power],
              N      = c(sum(id.ValidL),sum(id.ValidS)))
  )
}
...
# hauser3.1=morality judgment (greater good condition); hauser4.1=morality judgment (foreseen side-effect condition; for both, yes=1, no=2. haus3.2 and haus4.2=previous experience (yes=1, no=2); haus3.27_3=timing (greater good); haus4.2t_3=timing (side effect).
# rise1.3=likelihood that the professor will call on you (unprepared condition); rise2.3=likelihood that the professor will call on you (prepared condition); for both, higher numbers=higher likelihood
# sava1.N=interpersonal actions; sava2.N=personal actions; sava1.4=choice (buy a gift; 1=choice; 2=no choice); sava1.5=importance (buy a gift); sava1.9=choice (take a friend at the restaurant; 1=choice; 2=no choice); sava1.10=choice (restaurant); sava1.15=choice (trip; 1=choice, 2 and 3 = no choice); sava1.16=importance (trip); sava1.21=choice (dinner; 1=choice, 2 and 3 = no choice); sava1.22=importance (dinner); sava1.27=choice (errand; 1=choice, 2 and 3 = no choice); sava1.28=importance (errand); sava1.33=choice (help, 1=choice, 2 & 3 = no choice); sava1.34=importance (help); sava1.38=choice (advice, 1=choice, 2 & 3 = no choice); sava1.39=importance (advice); sava1.43=choice (friends, 1=choice, 2 & 3 = no choice); sava1.44=importance (friends); sava2.4=choice (buy for yourself; 1=choice; 2=no choice); sava2.5=importance (buy for yourself); sava2.9=choice (at the restaurant by yourself; 1=choice; 2=no choice); sava2.10=choice (restaurant by yourself); sava2.15=choice (trip alone; 1=choice, 2 and 3 = no choice); sava2.16=importance (trip alone); sava2.21=choice (out for dinner; 1=choice, 2 and 3 = no choice); sava2.22=importance (out for dinner); sava2.27=choice (errand for yourself; 1=choice, 2 and 3 = no choice); sava2.28=importance (errand for yourself); sava2.33=choice (ask for help, 1=choice, 2 & 3 = no choice); sava2.34=importance (ask for help); sava2.38=choice (take a course, 1=choice, 2 & 3 = no choice); sava2.39=importance (take a course); sava2.43=choice (friends, 1=choice, 2 & 3 = no choice); sava2.44=importance (friends);  For all importance items: higher numbers=higher importance
# "nore1.1 TO nore1.20 provide choices (""belong to"" condition); nore2.1 to nore2.20 provide choices (""similar to"" condition). 
# [WAIT FOR APPROVAL OF SCORING KEYS]"
# hsee1.1=generosity ($90 scarf condition); hsee2.1=generosity ($110 coat condition); for both, higher numbers=higher generosity
# Scenario 1: baby harms adult; gray1.2=responsibility (baby); gray1.3=intentionality (baby); gray1.4=pain (adult). Scenario 2: adult harms baby; gray2.2=responsibility (adult); gray2.3=intentionality (adult); gray2.4=pain (baby)
# zhon1.1= unethical condition; zhon2.1=ethical condition; zhon.dv.1_1 TO zhon.dv.1_10=desirability of products (both conditions); higher numbers=higher desirability; Products list: Post-it notes (zhon.dv.1_1), Dove shower soap (zhon.dv.1_2), Crest toothpaste (zhon.dv.1_3), Nantucket Nectars juice (zhon.dv.1_4), Energizer batteries (zhon.dv.1_5), Sony cd cases (zhon.dv.1_6), Windex glass cleaner (zhon.dv.1_7), Lysol countertop disinfectant (zhon.dv.1_8), Snickers candy bar (zhon.dv.1_9), Tide laundry detergent (zhon.dv.1_10)
# schw1.1=life sat (first); schw1.2=partner satisfaction (second); schw2.1=partner satisfaction (first); schw2.2=life sat (second). for all, higher numbers=higher satisfaction
# shaf1.1=choice (award condition; Parent A=1, Parent B=2); shaf2.1=choice (deny condition; Parent A=1, Parent B=2)
# zav1.1 TO zav1.13 provide COLD primes. zav2.1 TO zav2.13 provide HEAT primes. zav.dv.2=belief; zav.dv.3=concern; higher numbers=higher belief/concern.
...
convertES <- function(inference, keepSign=TRUE){     
  ...
  if(inference$stat.type=='z'){ CItext <- list(test='z',df=', sum(inference$stat.N))')}
  if(inference$stat.type=='t'){ CItext <- list(test='t',df=', inference$stat.df1)')}
  if(inference$stat.type=='f'){ CItext <- list(test='f',df=', inference$stat.df1,inference$stat.df2)')}
  if(inference$stat.type=='chisq'){ CItext <- list(test='X',df=', sum(inference$stat.N), inference$stat.df1)')}
  ...
  #FisherZvars<- list(ncp='ES.r[2][1]',ciL='ES.r[2][2]',ciU='ES.r[2][3]')
  vVars     <-  list(ncp='ES.r$r[[1]]',ciL='ES.r$r[[2]]',ciU='ES.r$r[[3]]')
  
  ES.d  <- ldply(NCPvars,function(ncp) data.frame(delta=eval(parse(text=paste0(CItext$test,'_d(',ncp,CItext$df))))) 
  ES.r  <- ldply(NCPvars,function(ncp) data.frame(rho=eval(parse(text=paste0(CItext$test,'_r(',ncp,CItext$df)))) )
  ES.OR <- ldply(logORvars,function(ncp) data.frame(OR=eval(parse(text=paste0('d_logOR(',ncp,')')))) )
  #ES.fZ <- ldply(FisherZvars,function(ncp) data.frame(fZ=eval(parse(text=paste0('atanh(',ncp,')')))) )
  #ES.v  <- ldply(vVars,function(ncp) data.frame(v=eval(parse(text=paste0('v.stat(n=sum(inference$stat.N), inference$stat.ncp.pval, Rsq=',ES.r$r[1]^2,')')))) )
  ...
  #   
  options(warn = -1)
  return(data.frame(cbind(dcast(melt(ES.d), 1 ~ variable+.id)[2:4],dcast(melt(ES.r), 1 ~ variable+.id)[2:4],dcast(melt(ES.OR), 1 ~ variable+.id)[2:4])))
}
...
check.args <- function(stat.type, stat.ncp, stat.df1, stat.df2, stat.n1, stat.n2, alpha, CL, compare, tails){# Check input arguments
  ...
  if(length(stat.type)!=1){
    ...
    if(!any(tolower(c("z","t","f","chisq","chi.sq","X^2","X2","r","pr","B"))%in%stat.type)){ 
      warning(paste("Unknown test statistic. Choose from: Z, t, F, chisq, r, B"),immediate.=T)
...
esComp <- function(x, df1, df2, N, esType){
